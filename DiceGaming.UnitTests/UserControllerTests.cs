...
namespace DiceGaming.UnitTests
{
    ...
    public class UserControllerTests
    {
        ...
        public async Task Register_CorrectInfo_RetunrsNewUserDataAndCreatedSatusCode()
        {
            ...
            Assert.IsTrue(content.Value.UserId == user.Id);
            Assert.IsTrue(content.Value.Username == user.Username);
            Assert.IsTrue(content.Value.FullName == user.FullName);
            Assert.IsTrue(content.Value.Email == user.Email);
        ...
        public async Task UpdateProfileData_CorrectData_ReturnsUpdatedInfoAndOkStatusCode()
        {
            ...
            dynamic content = response.Content;
            Assert.IsTrue(content.Value.UserId == user.Id);
            Assert.IsTrue(content.Value.FullName == updateRequest.FullName);
            Assert.IsTrue(content.Value.Email == updateRequest.Email);
        ...
        public async Task AddMoney_ValidData_ReturnsNewBalanceAndOKStatusCode()
        {
            ...
            //Assert
            dynamic content = response.Content;
            Assert.IsTrue(content.Value.NewBalance == addMoneyRequest.AddMoney);
        ...
        public async Task GetBalance_ExistingUser_ReturnsBalanceAndOKStatusCode()
        {
            ...
            //Assert
            dynamic content = response.Content;
            Assert.IsTrue(content.Value.Balance == user.Money);
