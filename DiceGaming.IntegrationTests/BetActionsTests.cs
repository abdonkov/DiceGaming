...
namespace DiceGaming.IntegrationTests
{
    [TestFixture]
    public class BetActionsTests
    {
        ...
        public async Task MakeABetWithValidDataThenGetTheNewBetToCheckInfoAndDeleteIt()
        {
            ...
            var bet = new BetDto()
            {
                ...
                Stake = int.Parse(jsonResponse["stake"]),
                Win = int.Parse(jsonResponse["win"]),
            ...
            // Arrange 2
            request = TestHelper.GenerateRequestMessage(url: "http://localhost/api/users/" + userIdString + "/bets/" + bet.Id.ToString(),
            ...
            Assert.IsTrue(int.Parse(jsonResponse["stake"]) == bet.Stake);
            Assert.IsTrue(int.Parse(jsonResponse["win"]) == bet.Win);
            ...
            using (var db = new DiceGamingDb())
            {
                var deletedBet = db.Bets.FirstOrDefault(b => b.Id == bet.Id);
        ...
        public async Task TryAllBetActionsWithInvalidTokenAndCheckIfUnauthorizedReturned()
        {
            ...
            // Arrange 3
            request = TestHelper.GenerateRequestMessage(url: "http://localhost/api/users/" + userIdString + "/bets/1",
                                                        method: HttpMethod.Get,
                                                        token: "invalidToken");
